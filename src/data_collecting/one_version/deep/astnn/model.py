...
class BatchTreeEncoder(nn.Module):
    ...
    def traverse_mul(self, node, batch_index):
        ...
        if not size:
            return None
        batch_current = torch.zeros(size, self.encode_dim, device=self.device)
        ...
        batch_current = self.W_c(batch_current.index_copy(0, torch.LongTensor(index).to(self.device),
                                                          self.embedding(torch.LongTensor(current_node).to(self.device))))
        ...
        for c in range(len(children)):
            ...
            if tree is not None:
                 batch_current += zeros.index_copy(0, torch.LongTensor(children_index[c]).to(self.device), tree)
        # batch_current = F.tanh(batch_current)
        batch_index = [i for i in batch_index if i is not -1]
        b_in = torch.LongTensor(batch_index).to(self.device)
        self.node_list.append(self.batch_node.index_copy(0, b_in, batch_current))
        return batch_current
    # @profile
    def forward(self, x, bs):
        self.batch_size = bs
        self.batch_node = torch.zeros(self.batch_size, self.encode_dim, device=self.device)
        self.node_list = []
...
class BatchProgramClassifier(nn.Module):
    ...
    # @profile
    def __init__(self, embedding_dim, hidden_dim, vocab_size, encode_dim, label_size, batch_size, use_gpu=True, pretrained_weight=None):
    # def __init__(self, embedding_dim, hidden_dim, vocab_size, encode_dim, label_size, batch_size, use_gpu=False, pretrained_weight=None):
    ...
    def get_zeros(self, num):
        zeros = torch.zeros(num, self.encode_dim, device=self.device)
        return zeros
